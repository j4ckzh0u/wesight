...
Release assets 面向最终用户下载。CI artifacts 作为维护者测试构建产物的临时入口。
...
3. `Build Platforms` workflow 会构建 macOS、Windows 和 Linux 安装包。
4. 构建产物会先上传为 workflow artifacts，方便临时测试。
5. workflow 会创建一个 draft GitHub Release，并附上安装包。
